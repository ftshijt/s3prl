...
class PhonemeT5(torch.nn.Module):
    def __init__(self, downsample_rate, use_tokenizer=False, use_t5_hidden=False, randomize=False, t5="phone"):
        super().__init__()
        if t5 == "phone":
            self.tokenizer = AutoTokenizer.from_pretrained("voidful/phoneme_byt5")
            self.model = AutoModelForSeq2SeqLM.from_pretrained("voidful/phoneme_byt5")
        elif t5 == "org":
            self.tokenizer = AutoTokenizer.from_pretrained("google/byt5-small")
            self.model = AutoModelForSeq2SeqLM.from_pretrained("google/byt5-small")
        else:
            raise NotImplementedError("not support t5 type: {}".format(t5))
        self.dict = {'ð': 'T',
                    'ə': '@',
                    'ʌ': 'u',
                    'v': 'v',
        ...
        self.use_t5_hidden = use_t5_hidden
        self.randomize = randomize
        if self.randomize:
            self.model = self.randomize_model(self.model)
    
    def forward(self, prediction, use_kmeans=False):
        if use_kmeans:
            tokens = prediction
        else:
        ...
        # exit(1)
        result = self.model.encoder(tokens, output_hidden_states=True,output_attentions=True)
        if False: # FIXME(jiatong)
            hidden_state = [hidden for hidden in result["hidden_states"]]
            results = {
                "last_hidden_state": result["last_hidden_state"],
                "hidden_states": hidden_state
            }
        else:
            hidden_state = [self.upsampler(hidden) for hidden in result["hidden_states"]]
            results = {
                "last_hidden_state": self.upsampler(result["last_hidden_state"]),
                "hidden_states": hidden_state
            }
        return results
...
class KmeansModel(torch.nn.Module):
    def __init__(self, use_kmeans):
        super().__init__()
        self.kmeans_model = joblib.load(open("/usr0/home/jiatongs/project/s3prl/s3prl/upstream/wav2vec_u_phoneme_t5/km{}.bin".format(use_kmeans), "rb"))
    ...
    def forward(self, features_batch):
        ...
        for i, feats in enumerate(features_batch):
            pred = self.kmeans_model.predict(feats.detach().cpu().numpy())
            preds.append(torch.tensor(pred))
        result =  torch.stack(preds, dim=0).to(device)
        return result
