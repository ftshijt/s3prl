...
from .wav2vec_u_t5 import Wav2VecUInfer, SimpleDict, PhonemeT5, KmeansModel
...
class UpstreamExpert(UpstreamBase):
    def __init__(self, joint_dict, ppg=False, hidden=False, use_tokenizer=False, use_t5_hidden=False, randomize=False, use_kmeans="", t5="phone", **kwargs):
        super().__init__(**kwargs)
        assert version.parse(fairseq.__version__) > version.parse(
            "0.10.2"
        ), "Please install the fairseq master branch."

        assert ppg or hidden or use_kmeans != ""
        ...
        self.T5 = PhonemeT5(self.u_downsample_rate, use_tokenizer=use_tokenizer, use_t5_hidden=use_t5_hidden, randomize=randomize, t5=t5)

        # Note(jiatong): Temporal clustering method
        if use_kmeans == "":
            self.use_kmeans = False
        else:
            self.use_kmeans = True
            self.kmeans_model = KmeansModel(use_kmeans)
        ...
        if len(self.hooks) == 0:
            ...
            if self.use_kmeans:
                # self.add_hook("self.kmeans_model", lambda input, output: output)
                assert not hidden and not ppg
            else:
                if hidden:
            ...
            self.add_hook("self.T5", lambda input, output: output["last_hidden_state"])
            
            if use_t5_hidden:
                for i in range(13): # TODO(Jiatong): should not fix the hidden
                    self.add_hook("self.T5", lambda input, output: output["hidden_states"][i])
        ...
        feature_num = 25 if not hidden else 26
        self.feature_groups = {"hidden_states": [[i for i in range(feature_num)]]}
        if ppg:
            self.feature_groups["hidden_states"].append([feature_num])
            feature_num = feature_num + 1
        if use_t5_hidden:
            self.feature_groups["hidden_states"].append([i + feature_num for i in range(14)])
        else:
            self.feature_groups["hidden_states"].append([feature_num])
    ...
    def forward(self, wavs):
        ...
        if self.use_kmeans:
            final_result = self.kmeans_model(u_feature.permute(1, 0, 2))
            results = self.T5(final_result, use_kmeans=True)
        else:
